...
from libcloud.common.vagrant import Vagrant, SandboxVagrant, make_file_cm, get_vagrant_executable, \
...
def list_box_names():
    ...
    """
    listing = ensure_string(subprocess.check_output('vagrant box list --machine-readable', shell=True))
    box_names = []
...
def test_vm_config():
    ...
    ssh_config = ensure_string(subprocess.check_output(command, cwd=TD, shell=True))
...
def test_vm_sandbox_mode():
    ...
    sahara_installed = _plugin_installed(Vagrant(TD), 'sahara')
...
def test_multivm_config():
    ...
    """
    v = Vagrant(TD, quiet_stdout=False, quiet_stderr=False)
    v.up(vm_name=VM_1)
...
def _execute_command_in_vm(v, command):
    ...
    return ensure_string(subprocess.check_output(ssh_command, cwd=v.root))
...
def _plugin_installed(v, plugin_name):
    ...
    return plugin_name in frozenset(plugin.name for plugin in plugins)
